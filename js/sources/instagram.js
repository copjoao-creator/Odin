/*
 * Fonte: Instagram Graph API (oficial da Meta).
...
Odin.Sources.instagram = (() => {
  ...
  async function get(p, path, params = {}) {
    const qs = new URLSearchParams({ ...params, access_token: p.token.trim() });
    let res;
    try {
      res = await fetch(`https://${p.apiHost}/${p.apiVersion}/${path}?${qs}`);
    } catch {
      throw new Err('Falha de rede ao acessar a API do Instagram. Verifique sua conexão.');
    }
    ...
    if (!res.ok || data.error) {
      ...
      if ([4, 17, 32, 613].includes(code)) {
        throw new Err('Limite de chamadas da API atingido. Aumente o intervalo ou reduza o número de publicações monitoradas.', code);
      }
  ...
  async function mediaInsights(p, media) {
    ...
    try {
      return parseInsights(await get(p, `${media.id}/insights`, { metric: wanted.join(',') }));
    } catch (e) {
      if (e.auth) throw e;
      ...
      for (const metric of wanted) {
        try {
          Object.assign(out, parseInsights(await get(p, `${media.id}/insights`, { metric })));
        } catch (e2) {
          if (e2.auth) throw e2;
  ...
  async function fetchAll(p, ctx) {
    if (!p.token || !p.token.trim()) {
      throw new Err('Informe o token de acesso do Instagram em Configurações.', 0, true);
    }
    const uid = (p.accountId || 'me').trim();

    const acc = await get(p, uid, { fields: 'id,username,followers_count,media_count,profile_picture_url' });
    const list = await get(p, `${uid}/media`, {
      fields: 'id,caption,media_type,media_product_type,permalink,thumbnail_url,media_url,timestamp,like_count,comments_count',
      limit: String(ctx.postsToWatch)
    });
    const media = list.data || [];
    const insights = await pool(media, 4, (m) => mediaInsights(p, m));
    ...
    return {
      ...
      posts: media.map((m, i) => {
        ...
        return {
          id: m.id,
          caption: m.caption || '',
          type: m.media_product_type === 'REELS' ? 'REELS' : m.media_type,
